...
mps.loadGPT = function() {
  if (mps._gptrefresh) {
    try {
      ...
      var gptsrc =  typeof (mps.response) === 'object' ? proto + mps.response.dart.params.gpt_js_uri : proto + 'securepubads.g.doubleclick.net/tag/js/gpt.js';
...
if(mps._pp.pathsegs[0]=='index.php') mps._pp.pathsegs.shift();
mps._pp.pathsegsold = mps._trim(mpscall.path,'/').split('/');
if(!mpscall.cat && !mps._pp.pathsegs[0]) mpscall.cat = 'home';
mps._pp.catsegs = mpscall.cat ? mpscall.cat.split('|') : [];

//--[2017-11-10] Set cag[adunit1] to cag[adunit6]
mps._pp.aua = mpscall.adunit.split('/');
for(var $auc=1; $auc<6; ++$auc) {
  mpscall['cag[adunit'+$auc+']'] = ($auc >= mps._pp.aua.length ? '' : mps._pp.aua[$auc]);
}
...
mpscall.ismobile = window.screen.width < 758 ? 'true': 'false';
console.warn('[mps:hb:config] window.screen.width = ' + window.screen.width);



//--> HEADER BIDDING CONFIG [nbcnews-bento]
mps.hb = mps.hb || {}; 
mps.hb=mps.hb||{};
mps.hb.customConfig = function(){
  if(mpscall.adunit.indexOf('/nbcnews/')===5) {
    mps.hb.ixjs = '185796-203819031605032.js';
  }
  if(mpscall.adunit.indexOf('/today/')===5) {
     //mps.hb.ixjs = '185796-206018056054159.js';
     // turn on magnite for today
     mps.hb.mgid = '24610';
     mps.hb.mgtimeout = 3000;

  }
if(mpscall.adunit.indexOf('/msnbc/')===5) {
    mps.hb.ixjs = '185796-12504009698505.js';
  }
}
/*=======HB VENDORS=======*/
mps.hb.a9pubid = '3219';
mps.hb.ixjs=mps.hb.ixjs||'';
mps.hb.moatyi = 'nbcuyieldheader7581548001';
/*=======OPTIONAL CONFIG=======*/
mps.hb.a9timeout = 2000;
mps.hb.ixtimeout = 3000;
mps.hb.moatyitimeout = 2000;
mps.hb.a9sisectoverride = '';
mps.hb.a9sizes =['300,250','300,600','728,90','970,66','970,90','970,90','970,250','1400,600','300,300'];
...
/**
 * Revised on 05/22/22 adding Magnite Prebid
 * Header Bidding (v9)
...
// move this to config
//mps.hb.mgid =   mps.hb.mgid || '24610';
//mps.hb.mgtimeout = 4000;
// For testing to remove IX
//mps.hb.ixjs = undefined;
//delete(mps.hb.ixjs);


mps.hb.mgtimeout = mps.hb.mgtimeout || 2000;
...
// a9 js url
mps.hb.a9js = mps.hb.a9js || "//c.amazon-adsystem.com/aax2/apstag.js";
...
mps.hb.execute = function() {
  ...
  if (mps._checkQS('DISABLEHB') || (!mps.hb.a9pubid && !mps.hb.ixjs && !mps.hb.moatyi && !mps.hb.mgid)) {
    mpsopts.skipdisableinitialload = 1;
...
mps.hb.executeCall = function() {
  ...
  mps._d('[mps:hb] Starting Execution v9.0 (CORE)');
  // Load moatyi
  if (mps.hb.moatyi) {
    mps.hb.loadMoatYI();
  }
  // Load ixjs
  if (mps.hb.ixjs && !mps.hb.mgid) {
    mps.hb.loadIX();
  }
  ...
  mps._queue.gptloaded.push(function() {
    if (mps.hb.a9pubid) {
      mps._d('[mps:hb:a9] A9: Assembling ad slot data');
      ...
      mps._d('^[mps:hb:a9] CONTINUE startAdExecution (skip A9)');
      mps.hb.startAdExecution();
    }
  });
  return true;
...
mps.hb.loadMoatYI = function(){
  try {
    ...
    mps.hb.moatyiurl = mps.hb.moatyiurl ||  'https://z.moatads.com/' + mps.hb.moatyi + '/moatheader.js';
    mps._loadJS(mps.hb.moatyiurl, function () {
      mps.hb._moatyiready = 1;
      mps._d('[mps:hb:moat] Loaded MoatYI: ('+mps.hb.moatyi+') ' +mps._elapsed());
      mps._reqs.hb.moatyi_loaded = mps._elapsed('', true);
    });
    setTimeout(function () {
      if (!mps.hb._moatyiready) {
        mps.hb._moatyiready = 1;
        mps._d('^[mps:hb:moat] CONTINUE startAdExecution (skip MOATYI, timeout:' + mps.hb.moatyitimeout + 'ms)');
        mps.hb.startAdExecution();
      }
    }, mps.hb.moatyitimeout);
    if (debugmode.log && window.moatPrebidApi && typeof(window.moatPrebidApi.enableLogging) === 'function') {
      mps._d('[mps:hb:moat] MoatYI: typeof(window.moatPrebidApi.enableLogging)');
    ...
    mps._queue.adexecute.push(function () {
      ...
      for (var i in mps._advars[loadset]) {
        ...
        if (!adobj || !adobj.getSlotElementId || !adobj.getAdUnitPath || !adobj.getSizes || !adobj.getTargetingMap){
          mps._d('[mps:hb:moat] MoatYI: skipped adobj missing data');
          continue;
        }
        var exclude = adobj !== null && adobj.getCategoryExclusions();
        if (exclude && exclude.indexOf('headerbid') > -1) {
          mps._d('[mps:hb:moat] MoatYI: skip cat exclusion of headerbidding '+ adobj.getCategoryExclusions());
          continue;
        }
        if (i === '_oop' || !adobj || !adobj.setTargeting){
          continue;
        }
        mps._d('[mps:hb:moat] MoatYI: pushing moat to slot ' + adobj.getSlotId());
        mps.hb.moatyislots.push(i);
      }
      if (window.moatPrebidApi && window.moatPrebidApi.pageDataAvailable()) {
        mps._d('[mps:hb:moat] MoatYI: Call setMoatTargetingForAllSlots for ' + mps.hb.moatyislots.length + ' ad slots');
        window.moatPrebidApi.setMoatTargetingForAllSlots();
      } else {
        mps._d('[mps:hb:moat] MoatYI: Call setMoatTargetingForAllSlots err window.moatPrebidApi.pageDataAvailable() doesnt exist');
      }
    });
  } catch(err) {
    mps._d('[mps:hb:moat] MoatYI: error');
 ...
 * @return {boolean}
 */
 mps.hb.loadMG = function () {
	mps._d('[mps:hb:mg] Trying to load Magnite...');
  try {
    if (mps.hb.mgid.split('.').pop() === 'js') {
      mps.hb.mgid = mps.hb.mgid.slice(0, -3);
    }
    if (!mps.hb._mgready) {
      mps.hb.mgurl = mps.hb.mgurl || '//micro.rubiconproject.com/prebid/dynamic/' + mps.hb.mgid + '.js';
      mps._reqs.hb.mg_begin = mps._elapsed('', true);
      mps._loadJS(mps.hb.mgurl, function () {
        mps._d('[mps:hb:mg] Magnite bids loading requesting bids');
        
				// request pbjs bids when it loads
        pbjs.que.push(function() {
					pbjs.rp.requestBids({
							callback: mps.hb.MGBidCallback
					});
			});
      });
      setTimeout(function () {
				if(!mps.hb.adserverCalled){
					mps.hb._mgready = 1;
					mps._d('^[mps:hb:mg] CONTINUE startAdExecution (skip MG, timeout:' + mps.hb.mgtimeout + 'ms)');
					mps.hb.startAdExecution();
				}
    ...
    mps._errorCallback(err, 'mps.hb.loadMG');
  }
  return true;
};

/** callback for Magnite bids 
 * Ensures bids are loaded prior to ad execution
 * @return {boolean}
*/
mps.hb.MGBidCallback = function () {
  mps.hb._mgready = 1;
  mps._d('[mps:hb:mg] Loaded Magnite: (' + mps.hb.mgid + ') ' + mps._elapsed());
  mps._reqs.hb.mg_loaded = mps._elapsed('', true);
  mps.hb.startAdExecution();
};
...
mps.hb.loadIX = function () {
  try {
    ...
    if (!mps.hb._ixready) {
      mps.hb.ixurl = mps.hb.ixurl || 'https://js-sec.indexww.com/ht/p/' + mps.hb.ixjs + '.js';
      mps._reqs.hb.ix_begin = mps._elapsed('', true);
      mps._loadJS(mps.hb.ixurl, function () {
        mps.hb._ixready = 1;
        mps._d('[mps:hb:ix] Loaded IX: (' + mps.hb.ixjs + ') ' + mps._elapsed());
      ...
      setTimeout(function () {
        if(!mps.hb.adserverCalled){
          mps.hb._ixready = 1;
          mps._d('^[mps:hb:ix] CONTINUE startAdExecution (skip IX, timeout:' + mps.hb.ixtimeout + 'ms)');
          mps.hb.startAdExecution();
        }
      }, mps.hb.ixtimeout);
...
mps.hb.loadA9 = function(){
  try {
    ...
    !function(a9,a,p,s,t,A,g){if(a[a9])return;function q(c,r){a[a9]._Q.push([c,r])}a[a9]={init:function() {q("i",arguments)},fetchBids:function(){q("f",arguments)},setDisplayBids:function(){},targetingKeys:function(){return[]},_Q: []};A=p.createElement(s);A.async=!0;A.src=t;g=p.getElementsByTagName(s)[0];g.parentNode.insertBefore(A,g)} ("apstag",window,document,"script",mps.hb.a9js);
    ...
    mps._d('[mps:hb:a9] A9: Setting SI Section ('+a9sisect+')');
    apstag.init({
...
mps.hb.a9BuildSlots = function (adobj) {
  ...
  for (var i in mps._advars[loadset]) {
    ...
    if(approvedSizesOnly === false){
      mps._d('^[mps:hb:a9] A9 skipped for slot : '+i+' : due to size ' + unapprovedSize);
      continue;
    }
    if( a9slotids.indexOf(slot.slotID) == -1 ){
      mps.hb.a9slots.push(slot);
    } else {
      a9slotids.push(slot.slotID );
      mps._d('[mps:hb:a9] EXCLUDED already called: '+i);
    }
  }
  if (mps.hb.nohb.a9 && mps.hb.nohb.a9.length) {
    mps._d('^[mps:hb:a9] EXCLUDED "nohb=a9" (' + mps.hb.nohb.a9.length + '): ' + mps.hb.nohb.a9.join(', '));
  }
...
mps.hb.a9FetchBids = function() {
  mps.hb.a9slots = mps.hb.a9slots || [];
  mps._d('[mps:hb:a9] A9: Passing ' + mps.hb.a9slots.length + ' ad slots');
  apstag.fetchBids({
    ...
    mps._d('[mps:hb:a9] Loaded A9: ('+mps.hb.a9pubid+') '+ mps._elapsed());
    mps._d('[mps:hb:a9] fetchBids A9: ' + JSON.stringify(bids.slice(bids.length-adcount, bids.length-1)));
...
mps.hb.startAdExecution = function() {
  ...
  mps.hb.previousloadset = loadset;
	if (mps.hb.mgid && !mps.hb._mgready) {
    mps._d('[mps:hb:mg] SKIP startAdExecution (waiting for mgready)');
    return false;
  }
  if ((mps.hb.ixjs && !mps.hb._ixready) && !mps.hb.mgid) {
    mps._d('[mps:hb:ix] SKIP startAdExecution (waiting for ixjs)');
    return false;
  }
  if (mps.hb.moatyi && !mps.hb._moatyiready) {
    mps._d('[mps:hb:moat] SKIP startAdExecution (waiting for moatyiready)');
    return false;
  }
  if (mps.hb.a9pubid && !mps.hb._a9ready) {
    mps._d('[mps:hb:a9] SKIP startAdExecution (waiting for a9ready)');
    return false;
  }
  if (mps.hb.adserverCalled === true && (mps.hb.previousloadset === mps._loadset)) {
  ...
  mps._queue.gptloaded.push(function() {
    ...
    if (mps.hb.excluded.length) {
      mps._d('^[mps:hb] EXCLUDED "!c=headerbid" INIT(' + mps.hb.excluded.length + '): ' + mps.hb.excluded.join(', '));
		...
		if (mps.hb.adserverCalled){ 
			mps._d('[mps:hb] Ads previously requested, skipping ad request.')
		} else {
			mps.hb.adserverCalled = true;
...
mps.execute_rails_js = function() {
  ...
  mps._pauseScroll = function(e) {
    mps._debug('^[mps:Rails:Core] PAUSE SCROLL!');
    e = e || window.event;
    if (e.preventDefault) {
      e.preventDefault();
    }
  ...
  mps._resumeScroll = function() {
    mps._debug('^[mps:Rails:Core] RESUME SCROLL >');
  ...
  mps._setInterscroller = function(railvars) {
    if(!railvars){
      mps._debug('[mps:Rails:Core] _setInterscroller empty railvars');
      return false;
    }
  ...
  mps._setRailsPadding = function(railvars){
    ...
    if(railvars._type = 'skin'){
      mps._debug('[mps:Rails:Core] _setRailsPadding PADTOP PADBOTTOM set from  PADTOP :'+railvars.PADTOP+' PADBOTTOM: '+railvars.PADBOTTOM);
      mps._debug('[mps:Rails:Core] _setRailsPadding PADTOP PADBOTTOM set to  PADTOP :'+railvars.railOffsetTop+' PADBOTTOM: '+railvars.railOffsetBottom);
      railvars.PADTOP = railvars.railOffsetTop;
      railvars.PADBOTTOM = railvars.railOffsetBottom;
  
    } 
    mps._debug('^[mps:Rails:Core] height: '+(creativeHeight)+' PADTOP: '+railvars.PADTOP+' PADBOTTOM: '+railvars.PADBOTTOM);
  ...
  if(typeof(mps.setRails) !== 'function'){
    mps.setRails = function (railvars) {
      ...
      mps._debug('[mps:Rails:Core] CALLED mps.setRails() '+mps._elapsed());
      ...
      if(typeof(railvars)!='object' || typeof(railvars.railAdBg)!='string' || (typeof(mps.disableRails)!='undefined' && mps.disableRails==0)) {
        mps._debug('[mps:Rails:Core] setRails Failed (undefined railvars, disabled rails or invalid selector)');
        return false;
      } else {
        mps._debug('[mps:Rails:Core] TRIGGER mps.setRails() '+mps._elapsed());
        ...
        if(typeof(mps.railvarsOverride)=='object' && typeof(mps._merge)=='function') {
          mps._debug('[mps:Rails:Core] OVERRIDE Merge (railvars + mps.railvarsOverride)');
        ...
        if(railvars.railContainerWidth && typeof(mps.pagevars.rails)=='object') {
          ...
          mps._debug('[mps:Rails:Core] USING mps.pagevars.rails '+JSON.stringify(mps.pagevars.rails));
        }
        if(railvars._type = 'skin'){
          mps._debug('[mps:Rails:Core] PADTOP PADBOTTOM set from  PADTOP :'+railvars.PADTOP+' PADBOTTOM: '+railvars.PADBOTTOM);
          mps._debug('[mps:Rails:Core] PADTOP PADBOTTOM set to  PADTOP :'+railvars.railOffsetTop+' PADBOTTOM: '+railvars.railOffsetBottom);
          railvars.PADTOP = railvars.railOffsetTop;
          railvars.PADBOTTOM = railvars.railOffsetBottom;
        ...
        mps._ready(function(){
          if(typeof(railvars._selector)!='object') {
            mps._debug('[mps:Rails:Core] FAILED setRails missing selector: '+railvars._target);
            return false;
          } else {
            // If mps-has-rails class exists return false.
            if(mps._classHas(railvars._selector, 'mps-has-rails')){
              mps._debug('[mps:Rails:Core] SKIP setRails (target already set)');
              return false;
            }
            ...
            if(!railvars.railContainerWidth) {
              ...
              if(railvars._target == 'body') {
                ...
                mps._debug('[mps:Rails:Core] OVERRIDE body selector');
              }
              ...
              if(mps._viewport()[0] < (adWidth * 2 + contentWidth) && !railVarsForce && !pagevarsForce) {
                mps._debug('[mps:Rails:Core] SKIP setRails (viewport too small)');
                return false;
              }
              ...
              mps._debug('[mps:Rails:Core] EXECUTED mps.setRails() '+mps._elapsed());
            ...
            if(typeof(top.mps._setRailsCallback)=='function') {
              if(typeof(top.mps)!='undefined' && typeof(top.mps._setRailsCallback)=='function') {
                delayms = 200;
                mps._debug('[mps:Rails:Core] TRIGGER setRailsCallback [delayed: '+delayms+'ms]');
                mps._gptcmd.push(function() {
                  setTimeout(function(){
            ...
            mps.loadrails++;
            mps._debug('[mps:Rails:Core] EXECUTED setRails() '+mps._elapsed());
          }
        });
      }
    ...
    mps._debug('[mps:Rails:Core] OVERRIDE: Using custom mps.setRails function');
  }
  
  // rails callback function for skins and fishtank only
  mps._setRailsCallback = function(railvars) {
    if(mps._isdebug()){
      console.log('[mps:Rails:Core] initial railvars', railvars)
    }
    if (railvars._type !== 'rails2' && typeof(railvars) === 'object' && typeof(railvars._selector) === 'object' && (railvars.railContainerHeight > 0 || railvars.railContainerHeight == 'full')) {
      ...
      if (railvars._type === 'rails1' || railvars._type === 'rails2') {
        ...
        mps._debug('[mps:Rails:Core] setRailsCallback FAILED');
      }
    }
  };
  ...
  if(typeof(mps.removeRails) !== 'function'){
    mps.removeRails = function(loadset) {
      ...
      if(typeof(mps.railvars[loadset])!='object' && typeof(mps.railvars[loadset])!=null) {
        mps._debug('[mps:Rails:Core] FAILED removeRails('+loadset+'): invalid loadset');
        return false;
      }
      ...
      if(typeof(mps.railvars[loadset]._selector) == 'object' && typeof(mps._prerailstate[loadset]) == 'object' && mps.loadrails > 0) {
        ...
        if(typeof(top.mps.removeRailsCallback)=='function') {
          mps._debug('[mps:Rails:Core] TRIGGER removeRailsCallback('+loadset+')');
        ...
        mps._debug('[mps:Rails:Core] EXECUTED removeRails('+loadset+')');
      } else {
        mps._debug('[mps:Rails:Core] FAILED removeRails('+loadset+'): missing pre-rails');
        return false;
      }
      return true;
    };
  } else {
    mps._debug('[mps:Rails:Core] OVERRIDE: Using custom mps.removeRails function');
  }
  ...
  mps._railsClick = function(e){
    ...
    if(mps.railvars[mps._loadrails]._target == 'body') {
      if((typeof targ.className!='undefined') && (targ.className.indexOf('mps-rails-'+mps._loadrails)!=-1)) {
        mps._debug('[mps:Rails:Core] Skipping Click: '+mps.railvars[mps._loadrails].railAdBgClickthru);
      }
    } else {
