...
function initializeTeaserLoader(d,h,g,f,a,j,b,k,c){f=!!(CQ.Ext&&(f=="true"||f===true));
if(window.CQ_Analytics){var e=function(){var o="/_jcr_content/par.html";
if(f){o+="?wcmmode=disabled"
...
for(var s=0;
s<d.length;
s++){var u=CQ.shared.HTTP.externalize(d[s].path+".html");
if(!d[s]["segments"]||d[s]["segments"].length==0||(CQ_Analytics.SegmentMgr.resolveArray(d[s]["segments"])===true&&checkMatch(d[s]["segments"]))){var q=CQ_Analytics.SegmentMgr.getMaxBoost(d[s]["segments"]);
var w=[d[s]["title"],q,d[s].thumbnail,u];
if(r==d[s].path){t+=CQ.I18n.getMessage('<b><a href="{3}" class="cq-teaser-segment-link"><img src="{2}" class="cq-teaser-decision-thumbnail cq-teaser-decision-match"></a>Experience: {0} - match ( boost = {1} )</b><br>',w)
...
x.push(d[s]);
v=q
}}}else{var w=[d[s]["title"],d[s].thumbnail,u];
...
var l=function(){var C=new Array();
if(CQ_Analytics.SegmentMgr){var x=0;
for(var u=0;
u<d.length;
u++){if(!d[u]["segments"]||d[u]["segments"].length==0||(CQ_Analytics.SegmentMgr.resolveArray(d[u]["segments"])===true&&checkMatch(d[u]["segments"]))){var z=CQ_Analytics.SegmentMgr.getMaxBoost(d[u]["segments"]);
if(z==x){C.push(d[u])
}else{if(z>x){C=new Array();
C.push(d[u]);
x=z
}}}}}var q=null;
var y=null;
var t=window.videoLength?(window.videoLength)*1000:0;
var F;
if(C.length>0){if(h=="cycling-banner"){var s=C.length;
var E;
var w=0;
F=k*1000;
var A=c*1000;
if(b>0){F=b*1000
}else{var v=Math.floor(t/C.length);
if(v>F){F=v
}}window.timeVar=null;
function B(){clearTimeout(window.timeVar);
if(w<C.length){E=C[w];
var G=E.path+o;
G=CQ.shared.HTTP.addSelectors(G,CQ.shared.HTTP.getSelectors(window.location.href));
CQ_Analytics.Utils.loadTeaserElement(G,g);
w++
}else{w=0
}window.timeVar=setTimeout(B,F)
}window.timeVar=setTimeout(B,A)
}else{var E=C[0];
if(CQ_Analytics.StrategyMgr){var D=CQ_Analytics.StrategyMgr.choose(h,C);
if(D!=null){E=D
}}if(!q||q.path!=E.path){q=E;
var p=E.path+o;
...
$CQ(window).unload(function(){try{var G=CQ_Analytics.loadedTeasersStack;
if(G){delete CQ_Analytics.loadedTeasersStack;
var I=a;
for(var J=0;
J<G.length;
J++){I=CQ.shared.HTTP.addParameter(I,"path",G[J])
}CQ.shared.HTTP.get(I,function(){})
}}catch(H){}})
}CQ_Analytics.loadedTeasersStack.push(E.path)
}if(f){if(j){var r=CQ.WCM.getEditable(j);
if(r){if(r&&r.teaserToolTip){r.teaserToolTip.hide();
r.teaserToolTip.remove();
r.teaserToolTip=null
}else{r.on(CQ.wcm.EditRollover.EVENT_SHOW_HIGHTLIGHT,function(G){if(!this.teaserInfoButton){this.teaserInfoButton=CQ.Ext.DomHelper.append("CQ",{tag:"div",cls:"x-tool x-tool-help cq-teaser-tooltip-tool"},true);
this.teaserInfoButton.position("absolute");
this.teaserInfoButton.on("click",function(){if(!r.teaserToolTip){r.teaserToolTip=new CQ.Ext.Tip({html:n(q.path),title:CQ.I18n.getMessage("Selection decision"),width:450,autoHide:false,closable:true,height:300,floating:true,autoHeight:false,bodyStyle:"overflow-y: scroll;"})
}var H=this.getXY();
r.teaserToolTip.setPosition(H[0]-460,H[1]-100);
r.teaserToolTip.show()
})
}this.teaserInfoButton.anchorTo(G.frameBottom.getEl(),"tr",[-26,-15]);
this.teaserInfoButton.show()
});
r.on(CQ.wcm.EditRollover.EVENT_HIDE_HIGHTLIGHT,function(G){if(this.teaserInfoButton){this.teaserInfoButton.hide()
}})
}}}}}}}else{if(f){var r=CQ.WCM.getEditable(j);
...
if(CQ_Analytics.CCM.areStoresInitialized){e.call(this)
}else{CQ_Analytics.CCM.addListener("storesinitialize",e)
}}}function checkMatch(b){var a=true;
if(b&&b.length>0){if(!CQ_Analytics.SegmentMgr.segments[b[0]]){a=false
}}return a
...
CQ_Analytics.StrategyMgr.register("random-custom",function(e){var b=Math.random();
if(parseFloat(b)>1){b=1/b
}if(parseFloat(b)==1){b=0
}var d=-1;
for(var a=0;
a<e.length;
a++){if(e[a]["name"]=="default"){d=a
}}if(d>-1){e.splice(d,1)
}var c=Math.floor(b*e.length);
return e[c]
});
CQ_Analytics.StrategyMgr.register("list-custom",function(a){return a
});
CQ_Analytics.Utils.isOptimizedCC=false;
CQ_Analytics.Utils.loadTeaserElement=function(b,h){var a=function(m){if(m&&m!=""){var k=b.replace(new RegExp("/","g"),"|").replace(".html","");
var l=$CQ(m).css("display","none");
var j=document.getElementById(h);
j.setAttribute("data-campaign-url",k);
$CQ("#"+h).html(l);
if(j&&j.parentElement){$CQ(j.parentElement).trigger("target-dom-loaded");
$CQ(window).trigger("resize")
}if(!this.resetHieghtTimer){window.setTimeout(function(){$CQ(".content-container .target:first-child").css("min-height","0px")
},50);
this.resetHieghtTimer=true
}l.fadeIn()
}};
var f=function(j,k){if(!CQ_Analytics.Utils.teasersCache){CQ_Analytics.Utils.teasersCache={}
}CQ_Analytics.Utils.teasersCache[j]=k
};
var g=function(){var k=CQ.shared.HTTP.getSelectors(b);
if(b.indexOf("globalheaderfooter")!=-1&&k.length!=0){var j=b.substring(0,b.indexOf(k[0]));
b=j.concat(k[0],".html");
if(typeof CQ.WCM!=="undefined"&&CQ.WCM.isEditMode){b=b+"?wcmmode=disabled"
}}if(CQ_Analytics.Utils.teasersCache&&CQ_Analytics.Utils.teasersCache[b]){a(CQ_Analytics.Utils.teasersCache[b])
}else{CQ_Analytics.Utils.teasersLoading=CQ_Analytics.Utils.teasersLoading||{};
if(CQ_Analytics.Utils.teasersLoading[b]){window.setTimeout(function(){CQ_Analytics.Utils.loadTeaserElement(b,h)
},100)
}else{CQ_Analytics.Utils.teasersLoading[b]=true;
e()
}}};
var e=function(){var k=b;
var j=location.href;
if(typeof CQ_CONTENT_PATH!="undefined"){j=CQ_CONTENT_PATH
}var l=CQ.shared.HTTP.getParameter(j,"wcmmode");
if(l){k+=(k.indexOf("?")>0?"&":"?")+"wcmmode="+l
}CQ.shared.HTTP.get(k,function(q,p,m){if(p){var n=m.body;
if(n){n=n.replace(new RegExp("\\n","ig"),"");
n=n.replace(new RegExp("\\r","ig"),"");
f(b,n);
delete CQ_Analytics.Utils.teasersLoading[b];
g()
}}else{f(b,"")
}})
};
var d=$CQ("#"+h).children().length;
if(d>0&&h.indexOf("globalheaderfooter")==-1&&!CQ_Analytics.Utils.isOptimizedCC){var c=0;
$CQ("#"+h).children().fadeOut(function(){var j=$CQ(this);
...
(function(){if(!CQ_Analytics.IMAppsStore){CQ_Analytics.IMAppsStore=CQ_Analytics.JSONPStore.registerNewInstance("imAppsStore");
CQ_Analytics.IMAppsStore.initialized=false;
CQ_Analytics.IMAppsStore.init=function(){var a=document.location.protocol+"//sync.im-apps.net/imid/segment?token=CmOnEerIz4AxiIkb8Mzm7Q";
$.ajax({url:a,type:"GET",dataType:"JSONP",success:function(d){if(d&&d.segment_eids){var b=[];
for(var c in d.segment_eids){if(typeof d.segment_eids[c]==="string"){b.push(d.segment_eids[c])
}}d.segment_eids=b.join(", ")
}CQ_Analytics.IMAppsStore.data=d;
...
},error:function(b,d,c){console.error("Ajax request towards Intimate Merger API failed: "+c)
},timeout:3000})
};
CQ_Analytics.IMAppsStore.getValue=function(a){if(CQ_Analytics.IMAppsStore.data&&CQ_Analytics.IMAppsStore.data[a]){return CQ_Analytics.IMAppsStore.data[a].value
}return""
}
}}());
(function(){if(!CQ_Analytics.kruxStore){CQ_Analytics.kruxStore=CQ_Analytics.JSONStore.registerNewInstance("kruxStore");
CQ_Analytics.kruxStore.initialized=false;
CQ_Analytics.kruxStore.supportsLocalStorage=function(){return typeof(Storage)!=="undefined"
};
CQ_Analytics.kruxStore.finishInit=function(){CQ_Analytics.kruxStore.initialized=true;
CQ_Analytics.kruxStore.fireEvent("initialize",CQ_Analytics.kruxStore);
CQ_Analytics.kruxStore.fireEvent("update")
};
CQ_Analytics.kruxStore.grabKruxDataFromLocalStorage=function(){var a=new Object();
a=CQ_Analytics.kruxStore.getLocalStoreProperties();
if(Object.keys(a).length===0||localStorage.length===0){a=CQ_Analytics.kruxStore.getDefaultKruxObject()
}CQ_Analytics.kruxStore.data=a
};
CQ_Analytics.kruxStore.getLocalStoreProperties=function(){var a=new Object();
var b="^kxsfdc_";
$.each(localStorage,function(d,e){if(d.toString().match(b)){var c=d.toString();
a[c]=e
}});
return a
};
CQ_Analytics.kruxStore.init=function(){if(CQ_Analytics.kruxStore.supportsLocalStorage()){CQ_Analytics.kruxStore.grabKruxDataFromLocalStorage();
CQ_Analytics.kruxStore.finishInit()
}};
CQ_Analytics.kruxStore.getValue=function(a){if(CQ_Analytics.kruxStore.data&&CQ_Analytics.kruxStore.data[a]){return CQ_Analytics.kruxStore.data[a].value
}return""
};
CQ_Analytics.kruxStore.getDefaultKruxObject=function(){var a={kxsfdc_apac_geo:"-",kxsfdc_apac_kuid:"-",kxsfdc_apac_segs:"-",kxsfdc_apac_tech:"-",kxsfdc_apac_user:"-",kxsfdc_us_geo:"-",kxsfdc_us_kuid:"-",kxsfdc_us_segs:"-",kxsfdc_us_tech:"-",kxsfdc_us_user:"-"};
return a
}
}}());
(function(){if(!CQ_Analytics.DreamforceSegmentUtils){CQ_Analytics.DreamforceSegmentUtils=CQ_Analytics.JSONStore.registerNewInstance("dreamforcesegment");
CQ_Analytics.DreamforceSegmentUtils.currentId="";
CQ_Analytics.DreamforceSegmentUtils.initialized=false;
var a=function(){for(var f in CQ_Analytics.DreamforceSegmentUtils.initProperty){if(CQ_Analytics.DreamforceSegmentUtils.initProperty.hasOwnProperty(f)){CQ_Analytics.DreamforceSegmentUtils.data[f]=CQ_Analytics.DreamforceSegmentUtils.initProperty[f]
...
var e=function(f){d(f,true)
};
var b=function(f){d(f,false)
};
var d=function(h,g){var f=new CustomEvent("dfdataloaded",{detail:"DF Visitor Profile data is set"});
$.ajax({type:"GET",url:h,success:function(j){CQ_Analytics.DreamforceSegmentUtils.data.visitorprofile=j.dfstatus;
CQ_Analytics.DreamforceSegmentUtils.data.eventId=j.eventId;
if(j.fname){CQ_Analytics.DreamforceSegmentUtils.data.fname=j.fname
}if(j.role){CQ_Analytics.DreamforceSegmentUtils.data.role=j.role
}a();
c(j);
document.dispatchEvent(f)
},async:g})
};
var c=function(g){if(g.reg_id!==undefined&&typeof digitalData!=="undefined"){var f=digitalData.util;
f.addPageData("tzregid",g.reg_id);
f.addBehavior("eventregidsuccess")
}};
CQ_Analytics.DreamforceSegmentUtils.init=function(){var k="guest",m=location.href,h=CQ.shared.HTTP.getParameter(m,"forceDreamforceProfile"),o=CQ.shared.HTTP.getParameter(m,"role"),l=CQ.shared.HTTP.getParameter(m,"fname"),f="?eventId="+$(".eventId").attr("val"),n="/wbin/dreamforce/v1/status"+f;
var g=location.hostname;
if("sfdc.local"===g||"localhost"===g){n="http://"+g+":8080"+n
}var j=false;
if(document.getElementById("regStatusCallType")){j=(document.getElementById("regStatusCallType").getAttribute("content")=="true")?true:false
}if(h){k=h;
CQ_Analytics.DreamforceSegmentUtils.data={visitorprofile:k,role:o,fname:l};
a()
}else{if(j){e(n)
}else{b(n)
}}}
}}());
if(!CQ_Analytics.VideoMetadataStore){CQ_Analytics.VideoMetadataStore=CQ_Analytics.JSONStore.registerNewInstance("videoMetadataStore");
CQ_Analytics.VideoMetadataStore.currentId="";
CQ_Analytics.VideoMetadataStore.init=function(){var g="";
var a=decodeURIComponent((new RegExp("[?|&]debugVideoId=([^&;]+?)(&|#|;|$)").exec(location.search)||[,""])[1].replace(/\+/g,"%20"))||null;
if(a){g=a
}else{if(typeof window.vidyardPlayerID!=="undefined"){g=window.vidyardPlayerID
}}var e="";
var f=window.eventprefix||"";
if(g!=""){e="https://a.sfdcstatic.com/"+f+"_video-detail-"+g+".json"
}else{e="https://a.sfdcstatic.com/df15_sflive-segments-template.json"
}if(e!=""){try{var d=null;
if($.browser.msie&&window.XDomainRequest){var b=new XDomainRequest();
b.onload=function(){d=$.parseJSON(b.responseText);
CQ_Analytics.VideoMetadataStore.initData(d)
};
b.onprogress=function(){};
b.open("get",e);
b.send()
}else{var h=function(j){d=j;
CQ_Analytics.VideoMetadataStore.initData(d)
};
$.getJSON(e,h)
}if(d){CQ_Analytics.VideoMetadataStore.initData(d)
}}catch(c){console.log("Error",c)
}}};
CQ_Analytics.VideoMetadataStore.initData=function(a){CQ_Analytics.VideoMetadataStore.data=a;
for(var b in CQ_Analytics.VideoMetadataStore.initProperty){if(CQ_Analytics.VideoMetadataStore.initProperty.hasOwnProperty(b)){CQ_Analytics.VideoMetadataStore.data[b]=CQ_Analytics.VideoMetadataStore.initProperty[b]
...
CQ_Analytics.VideoMetadataStore.getValue=function(a){if(CQ_Analytics.VideoMetadataStore.data){if(CQ_Analytics.VideoMetadataStore.data[a]){return CQ_Analytics.VideoMetadataStore.data[a].value
}}return""
}
}(function(){if(!CQ_Analytics.SFDCSegmentUtils){CQ_Analytics.SFDCSegmentUtils=CQ_Analytics.JSONStore.registerNewInstance("sfdcsegment");
CQ_Analytics.SFDCSegmentUtils.currentId="";
CQ_Analytics.SFDCSegmentUtils.initialized=false;
CQ_Analytics.SFDCSegmentUtils.init=function(){var a=(typeof vp!=="undefined"&&vp.isCustomer())?"customer":"prospect",c=location.href,b=CQ.shared.HTTP.getParameter(c,"forceProfile"),e=["customer","prospect"];
if(b&&(e.indexOf(b)>=0)){a=b
}CQ_Analytics.SFDCSegmentUtils.data={visitorprofile:a};
for(var d in CQ_Analytics.SFDCSegmentUtils.initProperty){if(CQ_Analytics.SFDCSegmentUtils.initProperty.hasOwnProperty(d)){CQ_Analytics.SFDCSegmentUtils.data[d]=CQ_Analytics.SFDCSegmentUtils.initProperty[d]
...
(function(){if(!CQ_Analytics.DemandbaseStore){CQ_Analytics.DemandbaseStore=CQ_Analytics.JSONStore.registerNewInstance("demandbaseStore");
CQ_Analytics.DemandbaseStore.currentId="";
CQ_Analytics.DemandbaseStore.initialized=false;
CQ_Analytics.DemandbaseStore.init=function(){CQ_Analytics.DemandbaseStore.data=typeof vp!=="undefined"&&vp.db;
if(!CQ_Analytics.DemandbaseStore.data){var d=document.location.protocol,c=CQ.shared.HTTP.getParameter(document.location.href,"ipOverride"),b=d+"//api.company-target.com/api/v2/ip.json?token=09dd7a8bed725339af3ce985f86800730b42b5cf"+((c!=""&&c!=null)?"&query="+c:"");
$.get(b).done(function(e){CQ_Analytics.DemandbaseStore.data=e;
a()
})
}else{a()
}};
CQ_Analytics.DemandbaseStore.getValue=function(b){if(CQ_Analytics.DemandbaseStore.data&&CQ_Analytics.DemandbaseStore.data[b]){return CQ_Analytics.DemandbaseStore.data[b].value
}return""
};
var a=function(){for(var b in CQ_Analytics.DemandbaseStore.initProperty){if(CQ_Analytics.DemandbaseStore.initProperty.hasOwnProperty(b)){CQ_Analytics.DemandbaseStore.data[b]=CQ_Analytics.DemandbaseStore.initProperty[b]
